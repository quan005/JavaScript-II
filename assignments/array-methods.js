...
runners.filter(function (runner) {
  ...
  if (runner["shirt_size"] === "L") {
    ...
    largeShirts.push("first_name: " + name + ", " + "shirt_size: " + size);
  }
});
...
let price = runners.reduce((donation, runner, index, runners) => {
...
// Problem 1 --- create an array of the runners emails and log the result
let runnerEmails = [];

runners.forEach(function (runner) {
  email = runner["email"];

  runnerEmails.push(email);
});

console.log(runnerEmails);

// Problem 2 --- Highest donator company name and donation then log results
let highestDonator = [];

var res = Math.max.apply(Math,runners.map(function(o){
  return o.donation;
}))

var obj = runners.find(function(o){

  if (o.donation == res){
    let company = o.company_name;
    highestDonator.push(company + " has made the biggest donation of " + res);
  }
})

console.log(highestDonator);

// Problem 3 --- list of all the companies that donated
let Comps = [];
    Comps = runners.map(function(currentValue) {
